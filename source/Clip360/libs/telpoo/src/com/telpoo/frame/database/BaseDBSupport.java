...
public class BaseDBSupport extends SQLiteOpenHelper {
	 ...
	 * @
	 */
	protected synchronized static boolean addList(ArrayList<BaseObject> pl, String tableName, String idKey) {
		if (pl == null)
			return false;
		try {
	...
	// success
	protected synchronized static boolean addListNoCheck(ArrayList<BaseObject> pl, String tableName) {
		if (pl == null)
			return false;
		try {
	...
	protected static int delete(String tableName, String selection, String[] selectionArgs) {
		...
		Mlog.D("delete db:" + selection + "--" + selectionArgs[0]);
		try {
			openDB();
			int deletedRows = mSqliteDatabase.delete(tableName, selection + " IN (?)", selectionArgs); // mSqliteDatabase.delete(tableName,
																										// selection,
																										// selectionArgs);
			closeDB();
			return deletedRows;
